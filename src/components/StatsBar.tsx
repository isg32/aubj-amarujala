...
const StatsBar = () => {
  return (
    <section className="gradient-stats py-10">
      <div className="container grid grid-cols-2 md:grid-cols-4 gap-8">
        {stats.map((stat) => (
          <div key={stat.label} className="text-center animate-count-up">
            <p className="text-3xl md:text-4xl font-heading font-bold text-accent mb-1">{stat.value}</p>
            <p className="text-sm font-body text-primary-foreground/80">{stat.label}</p>
          </div>
        ))}
      </div>
    </section>
